...
app.get('/searchquerry', async function(req, res, next) {
    ...
    var sort = { title: 1 };
    var data = await col.find({'title': {'$regex': req.query.title.toLowerCase()}}).sort(sort).toArray();

    var händlerlist = [];
    for( i = 0; i < data.length ; i++){
        try {
            console.log(data[i].title);
            if(0.1 <= similarity(data[i].title, req.query.title.toLowerCase())){ //if(0.3 <= stringSimilarity.compareTwoStrings(data[i].title, req.query.title.toLowerCase())){
    ...
    res.render('list', {tabledata: händlerlist, searchquery: req.query.title});
    }
});
...
app.get('/:PRODUCTID', async function(req, res, next) {
    ...
    for( i = 0; i < data2.length ; i++){
        try {
            ...
            if(0.78 <= similarity(data2[i].title, data[0].title.toLowerCase())){ //if(0.3 <= stringSimilarity.compareTwoStrings(data[i].title, req.query.title.toLowerCase())){
    ...
    for( i = 0; i < data2.length ; i++){
        try {
            //console.log(data2[i].title);
            //console.log(similarity(data2[i].title, data[0].title.toLowerCase()));
            if(0.77 > similarity(data2[i].title, data[0].title.toLowerCase()) && 0.5 < similarity(data2[i].title, data[0].title.toLowerCase())){ 
    ...
    */
    ...
    res.render('product', {tabledata: data[0], new_data: händlerlist, similar: reversed_sorted_data});
});
